...
fluid.defaults("gpii.pcpChannel", {
    ...
    invokers: {
        hackInferredConfiguration: {
            funcName: "gpii.pcpChannel.hackInferredConfiguration",
...
gpii.pcpChannel.receiveMessage = function (message, pcpChannel) {
    ...
    if (message.demoHack) {
        return pcpChannel.hackInferredConfiguration(message);
    }
...
/**
 * A hack to apply any setting.
 *
 * It's to quickly demonstrate applying settings via a key-pad, and should not be merged into master.
 *
 * @param pcpChannel
 * @param lifecycleManager
 * @param message
 */
gpii.pcpChannel.hackInferredConfiguration = function (pcpChannel, lifecycleManager, message) {
    var session = lifecycleManager.getSession(lifecycleManager.getActiveSessionTokens());

    var prefs = {};
    fluid.each(message.applications, function (app) {
        fluid.extend(prefs, app.settings);
    });

    session.applier.change(["preferences", "contexts", session.model.activeContextName, "preferences"], prefs);

    session.applier.change(["activeConfiguration", "inferredConfiguration", "applications"], message.applications, "ADD");

    // get new inferred configuration to be applied:
    var newInferred = gpii.matchMakerFramework.utils.updateInferredConfiguration(session.model.preferences, session.model.matchMakerOutput.inferredConfiguration, session.model.solutionsRegistryEntries);

    fluid.extend(newInferred[session.model.activeContextName].applications, message.applications);
    ...
    lifecycleManager.update(fullPayload).then(pcpChannel.events.preferencesApplied.fire);
};

// Warn about the above code
fluid.logObjectRenderChars(fluid.logLevel.WARN, "** WARNING: hackInferredConfiguration");
